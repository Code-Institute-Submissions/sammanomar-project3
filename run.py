...
class Board():
    ...
    def __init__(self):
        ...
        self.turns = 0
        # [r, c] invalid move because we haven't played anything yet
        self.last_move = [-1, -1]
    ...
    def turn(self, column):
        ...
        for i in range(ROW_COUNT-1, -1, -1):
            if self.board[i][column] == ' ':
                # execute this turn and put X or O here
    ...
    def check_winner(self):
        ...
        for a in range(4):
            for d in directions:
                ...
                if (d[2] and self.in_bounds(r, c) and
                        self.board[r][c] == last_letter):
                    d[1] += 1
                else:
...
def play():
    ...
    while not game_over:
        ...
        while not valid_move:
            try:
                user_move = int(input(f"{game.which_turn()}'s Turn - \
pick a column (1-{COLUMN_COUNT}): "))
                if user_move <= 0 or user_move > (COLUMN_COUNT):
...
"""set up game loop with python trick that
executes play function when file is run"""
